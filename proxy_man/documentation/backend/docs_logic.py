...
class DocumentationService:
    ...
    def get_item_order(self, path: str, parent_path: str = "") -> Tuple[int, str]:
        ...
        path_without_ext = path.replace(".mdx", "") if path.endswith(".mdx") else path
        ...
        for item in self.order_config.get("order", []):
            ...
            if not parent_path:
                # For root level files like "examples.mdx" or directories like "getting-started"
                if item_path == path_without_ext or item_path == f"{path_without_ext}.mdx":
                    return (item.get("order", default_order), path)

            # Check for items within a parent directory - match parent path exactly or with parent hierarchy
            if parent_path:
                # Clean parent path - remove leading slashes if any
                clean_parent = parent_path.lstrip('/')
                if item_path == clean_parent or item_path == parent_path:
                    # Look through children for this specific file
                    for i, child in enumerate(item.get("children", [])):
                        child_path = child.get("path", "")
                        # Match with or without .mdx extension
                        if child_path == path_without_ext or child_path == f"{path_without_ext}.mdx":
    ...
    def get_navigation_structure(self) -> List[Dict[str, Any]]:
        ...
        # Process files without sorting them here - sorting will be done in dict_to_list
        for mdx_file in mdx_files:
            ...
            # Special handling for index.mdx files - use parent directory as ID
            if parts[-1] == "index.mdx" and len(parts) > 1:
                # For folder/index.mdx, use "folder" as the ID
                doc_id = "_".join(parts[:-1])
            else:
                # For regular files, use the full path
        ...
        def dict_to_list(items_dict, parent_path=""):
            ...
            # Reorganize items to group index.mdx with its siblings
            reorganized = {}
            for key, item in items_dict.items():
                if item.get("is_directory"):
                    # Check if this directory has an index.mdx
                    if "index.mdx" in item.get("children", {}):
                        index_item = item["children"]["index.mdx"]
                        # Create a directory item that represents the index page
                        dir_item = {
                            "id": index_item["id"],  # Use the index.mdx's ID
                            "title": index_item["title"],  # Use the index.mdx's title
                            "path": index_item["path"],  # Use the index.mdx's path
                            "children": []
                        }
                        # Add non-index children
                        for child_key, child_item in item["children"].items():
                            if child_key != "index.mdx":
                                dir_item["children"].append({
                                    "id": child_item["id"],
                                    "title": child_item["title"],
                                    "path": child_item["path"],
                                    "children": child_item.get("children", [])
                                })
                        # Sort children by order
                        dir_item["children"].sort(
                            key=lambda x: self.get_item_order(
                                x["path"].split("/")[-1].replace(".mdx", ""),
                                key.replace(".mdx", "")
                            )
                        )
                        reorganized[key] = dir_item
                    else:
                        # Directory without index.mdx - shouldn't be clickable
                        reorganized[key] = {
                            "id": item["id"],
                            "title": item["title"],
                            "path": "",  # Empty path means not clickable
                            "children": dict_to_list(item["children"], key) if item["children"] else []
                        }
                else:
                    # Regular file (not in a directory)
                    reorganized[key] = item

            # Sort items by order configuration
            sorted_items = sorted(
                reorganized.items(),
                key=lambda x: self.get_item_order(x[0].replace(".mdx", ""), parent_path)
            )
    ...
    def get_page_content(self, page_id: str) -> Optional[Dict[str, Any]]:
        ...
        # Check if this might be a directory with index.mdx
        file_path = page_id.replace("_", "/") + ".mdx"
        full_path = self.docs_dir / file_path

        if not full_path.exists():
            # Try with index.mdx
            file_path = page_id.replace("_", "/") + "/index.mdx"
            full_path = self.docs_dir / file_path
