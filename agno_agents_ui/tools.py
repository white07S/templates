...
class ToolRegistry:
    """Registry for managing OpenAI function calling tools with JSON schema format."""
    ...
    def register(self, func: Callable) -> Callable:
        """Register a function as a tool with JSON schema format."""
        func_name = func.__name__
        self.tool_functions[func_name] = func

        # Generate and add schema to tools list
        schema = self._generate_json_schema(func)

        # Check if tool already exists and update, otherwise append
        existing_index = None
        for i, tool in enumerate(self.tools_schema):
            if tool["function"]["name"] == func_name:
                existing_index = i
                break

        if existing_index is not None:
            self.tools_schema[existing_index] = schema
        else:
    ...
    def _generate_json_schema(self, func: Callable) -> Dict[str, Any]:
        """Generate JSON schema format for OpenAI function calling."""
    ...
    def get_tools(self) -> List[Dict[str, Any]]:
        """Get all tool schemas for OpenAI API in JSON format."""
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        """Execute a tool with given arguments and optional caching."""
        if tool_name not in self.tool_functions:
            raise ValueError(f"Tool '{tool_name}' not found")

        # Generate cache key for deterministic tools
        if self._cache_enabled and self._is_cacheable_tool(tool_name):
    ...
    def _is_cacheable_tool(self, tool_name: str) -> bool:
        ...
        # Don't cache time-sensitive or stateful tools
        non_cacheable = {'get_current_time', 'random_fact'}
    ...
    async def execute_tools_parallel(self, tool_calls: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Execute multiple tools in parallel and return results for LLM continuation."""
        if not tool_calls:
            return []
        ...
        results = []
        for task, call_id, tool_name in tasks:
            try:
                result = await task
                print(f"✅ Tool '{tool_name}' completed: {str(result)[:100]}")

                # Format result for LLM continuation
                results.append({
...
class OptimizedToolExecutor:
    """High-level tool executor with two-step workflow and performance optimizations."""

    def __init__(self):
        self.registry = registry

    def get_available_tools(self) -> List[Dict[str, Any]]:
        """Get all available tools for OpenAI API in JSON schema format."""
    ...
    async def execute_tool_call(self, tool_name: str, arguments: str) -> Any:
        ...
        try:
            ...
            print(f"🔧 Tool '{tool_name}' executed with result: {str(result)[:100]}")
            return result
    ...
    async def process_tool_calls_for_llm(self, tool_calls) -> List[Dict[str, Any]]:
        """Process tool calls and format results for LLM continuation (two-step workflow)."""
        if not tool_calls:
            return []

        print(f"🔧 Processing {len(tool_calls)} tool calls for LLM continuation...")

        # Execute tools in parallel and prepare results for LLM
        results = await self.registry.execute_tools_parallel(tool_calls)

        print(f"✅ Completed {len(results)} tool executions for LLM")
        return results
