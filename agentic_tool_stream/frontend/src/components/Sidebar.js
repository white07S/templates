...
import { Search, MessageSquare, Plus, ChevronLeft, ChevronRight, Menu } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const Sidebar = ({ sessions, currentSessionId, onSessionSelect, onNewSession, isOpen, onToggle }) => {
  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={onToggle}
          className="fixed left-0 top-4 z-50 p-2 bg-white border-2 border-black border-l-0 rounded-r-lg hover:bg-gray-100 transition-colors"
        >
          <Menu size={20} />
        </button>
      )}
      ...
      <motion.div
        ...
        className="w-80 h-full bg-white border-r-2 border-black flex flex-col fixed left-0 top-0 z-40"
      >
      <div className="p-4 border-b-2 border-black">
        <div className="flex items-center gap-2 mb-3">
          <button
            onClick={onNewSession}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-red-600 text-white hover:bg-red-700 transition-colors"
          >
            <Plus size={20} />
      ...
      {isOpen && <div className="w-80 flex-shrink-0" />}
    </>
  );
};
