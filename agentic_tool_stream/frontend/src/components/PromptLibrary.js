...
const PromptLibrary = ({ user = 'default_user', onSelectPrompt }) => {
  ...
  const loadPrompts = async () => {
    setIsLoading(true);
    try {
      const data = await api.listPrompts({}, user);
      setPrompts(data);
      setError('');
    } catch (error) {
  ...
  const handleCreatePrompt = async (promptData) => {
    try {
      await api.createPrompt(promptData, user);
  ...
  const handleUpdatePrompt = async (promptId, updateData) => {
    try {
      await api.updatePrompt(promptId, updateData, user);
  ...
  const handleCopyPrompt = async (promptId) => {
    try {
      const result = await api.copyPrompt(promptId, user);
