...
async def create_prompt(req: PromptCreate, user_id: str = "john.doe"):
    try:
        # Validate prompt
...
async def list_prompts(user_created: Optional[bool] = None, keywords: Optional[str] = None, user_id: str = "john.doe"):
    try:
...
async def get_prompt(prompt_id: str, user_id: str = "john.doe"):
    try:
...
async def update_prompt(prompt_id: str, req: PromptUpdate, user_id: str = "john.doe"):
    try:
        prompt = prompts_table.get(PromptQ.id == prompt_id)
        if not prompt:
            raise HTTPException(status_code=404, detail="Prompt not found")
...
async def delete_prompt(prompt_id: str, user_id: str = "john.doe"):
    try:
        prompt = prompts_table.get(PromptQ.id == prompt_id)
        if not prompt:
            raise HTTPException(status_code=404, detail="Prompt not found")
...
async def copy_prompt(req: PromptCopy, user_id: str = "john.doe"):
    try:
