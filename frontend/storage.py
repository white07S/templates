...
class ConversationStore(TinyDBStore):
    ...
    def __init__(self, user_id: Optional[str] = None):
        ...
        if user_id:
            user_data_dir = settings.data_dir / f"user_{user_id}"
            user_data_dir.mkdir(exist_ok=True, parents=True)
            db_path = user_data_dir / "conversations.json"
        else:
            # Fallback to global conversations (for backward compatibility)
            db_path = settings.tinydb_conversations_path
    ...
    async def get_user_conversations(self, user_id: str, limit: int = 10) -> List[ConversationDocument]:
        ...
        # Force a fresh read from disk by reloading the table
        self.conversations_table = self.db.table('conversations')
...
class MemoryStore(TinyDBStore):
    ...
    def __init__(self, user_id: Optional[str] = None):
        ...
        if user_id:
            user_data_dir = settings.data_dir / f"user_{user_id}"
            user_data_dir.mkdir(exist_ok=True, parents=True)
            db_path = user_data_dir / "memories.json"
        else:
            # Fallback to global memories (for backward compatibility)
            db_path = settings.tinydb_memories_path
